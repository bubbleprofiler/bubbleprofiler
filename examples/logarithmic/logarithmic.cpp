...
/**
    ...
    in which exact solution is known (the logarithmic potential).
...
Logarithmic_options parse_cmd_line_args(int argc, const char* argv[])
{
   Logarithmic_options options;
...
int main(int argc, const char* argv[])
{
   ...
   int exit_code = 0;
   try {
      Logarithmic_options options = parse_cmd_line_args(argc, argv);

      if (options.global_min <= 0.) {
         options.global_min = 10. * std::abs(options.w);
      }
      ...
      Logarithmic_potential potential(options.m, options.w);
